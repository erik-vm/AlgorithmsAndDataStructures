...
public class QuaternionTest {
   ...
   @Test
   public void testPowZero() throws CloneNotSupportedException {
      Quaternion q = new Quaternion(2, 3, 4, 5);
      Quaternion expected = new Quaternion(1, 0, 0, 0);
      assertEquals("q.pow(0) peab olema 1", expected, q.pow(0));
   }

   @Test
   public void testPowOne() throws CloneNotSupportedException {
      Quaternion q = new Quaternion(2, 3, 4, 5);
      Quaternion result = q.pow(1);

      assertEquals("q.pow(1) peab olema võrdne q-ga", q, result);
      assertNotSame("q.pow(1) peab olema uus isend", q, result);
   }

   @Test
   public void testPowMinusOne() throws CloneNotSupportedException {
      Quaternion q = new Quaternion(1, 2, 3, 4);
      Quaternion expected = q.inverse();

      assertEquals("q.pow(-1) peab olema q.inverse()", expected, q.pow(-1));
   }

   @Test
   public void testPowPositive() throws CloneNotSupportedException {
      Quaternion q = new Quaternion(1, 0, 1, 0);
      Quaternion expected = q.times(q.pow(2)); // q^3 = q * q^2
      assertEquals("q.pow(3) peab olema q * q.pow(2)", expected, q.pow(3));
   }

   @Test
   public void testPowNegative() throws CloneNotSupportedException {
      Quaternion q = new Quaternion(1, 0, 1, 0);
      Quaternion expected = q.pow(2).inverse(); // q.pow(-2) = q.pow(2).inverse()
      assertEquals("q.pow(-2) peab olema q.pow(2).inverse()", expected, q.pow(-2));
   }

   @Test(expected = IllegalArgumentException.class)
   public void testPowNegativeZeroQuaternionThrows() throws CloneNotSupportedException {
      Quaternion zero = new Quaternion(0, 0, 0, 0);
      zero.pow(-1); // peab viskama erindi
   }
