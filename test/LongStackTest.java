...
public class LongStackTest {
    ...
    public void testInterpret2() {
        ...
        assertEquals("expression: " + s, 3, LongStack.interpret(s));
        s = "2 5 9 ROT - +";
        assertEquals("expression: " + s, 12, LongStack.interpret(s));
        s = "3 DUP *";
        assertEquals("expression: " + s, 9, LongStack.interpret(s));
        s = "3 7 DROP";
        assertEquals("expression: " + s, 3, LongStack.interpret(s));
        s = "9 11 17 ROT - SWAP -";
        assertEquals("expression: " + s, -3, LongStack.interpret(s));
        s = "-3 -5 -7 ROT - SWAP DUP * +";
        assertEquals("expression: " + s, 21, LongStack.interpret(s));
        s = "9 11 17 DUP ROT - SWAP DROP -";
        assertEquals("expression: " + s, 3, LongStack.interpret(s));
    }

    @Test(expected = RuntimeException.class)
    public void testInterpretNotEnoughElementsInSwap() {
        String s = "2 SWAP";
        LongStack.interpret(s);
    }

    @Test(expected = RuntimeException.class)
    public void testInterpretNotEnoughElementsInRot() {
        String s = "2 9 ROT";
        LongStack.interpret(s);
    }

    @Test(expected = RuntimeException.class)
    public void testInterpretNotEnoughElementsInDup() {
        String s = "SWAP";
        LongStack.interpret(s);
    }

    @Test(expected = RuntimeException.class)
    public void testInterpretNotEnoughElementsInDrop() {
        String s = "DROP";
        LongStack.interpret(s);
    }
